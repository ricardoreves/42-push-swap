...
class PSVizApp:
    ...
    def clickRun(self) -> None:
        ...
        try:
            self.__lstCmd = [""] + [line.decode("utf-8") for line in check_output(
...
class PSVizConfig:
    ...
    def __run_parser(self) -> None:
        ...
        self.parser.add_argument(
            ...
            metavar="FPS",
            type=int,
            help="frame speed refresh (default: {})".format(
                self.__speed),
        ...
        self.parser.add_argument(
            ...
            metavar="INT",
            type=int,
            help="min value for stack generator (default: {})".format(
                self.__min),
            default=self.__min,
            )
        ...
        self.parser.add_argument(
            ...
            metavar="INT",
            type=int,
            help="max value for stack generator (default: {})".format(
                self.__max),
            default=self.__max,
            )
        ...
        self.parser.add_argument(
            ...
            metavar="INT",
            type=int,
            help="size value for stack generator (default: {})".format(
                self.__size),
            default=self.__size,
            )
    ...
    def timeout(self, t : int) -> None:
        if (t < self.CMD_TIMEOUT_MIN or t > self.CMD_TIMEOUT_MAX):
            self.__error("timeout value between {} and {}".format(
...
class PSVizView:
    ...
    def windowMain(self) -> None:
        ...
        self.__lblFrame         = Label(self.__fmtCtrl,
                                        text="frame:",
                                        font=self.FONT_SMALL)
        self.__strFramePos      = StringVar(value="0")
        self.__entFramePos      = Entry(self.__fmtCtrl,
                                        state="disable",
        ...
        self.__lblSep           = Label(self.__fmtCtrl,
                                        text="/",
                                        font=self.FONT_SMALL)
        self.__strFrameNum      = StringVar(value="0")
        self.__entFrameNum      = Entry(self.__fmtCtrl,
                                        state="disable",
                                        width=self.ENT_CTRL_WIDTH,
        ...
        self.__lblFrameSpeed    = Label(self.__fmtCtrl,
                                        text=" speed:",
                                        font=self.FONT_SMALL)
        self.__strFrameSpeed    = StringVar()
        self.__entFrameSpeed    = Entry(self.__fmtCtrl,
                                        width=self.ENT_CTRL_WIDTH,
        ...
        self.__btnNext          = Button(self.__fmtCtrl,
                                        text=">>",
                                        command=self.app.clickNext,
                                        width=self.BTN_CTRL_WIDTH,
                                        state="disable"
                                        )
        self.__btnPlay          = Button(self.__fmtCtrl,
                                        text=">",
                                        command=self.app.clickPlay,
                                        width=self.BTN_CTRL_WIDTH,
                                        state="disable",
                                        )
        self.__btnPrev          = Button(self.__fmtCtrl,
                                        text="<<",
        ...
        self.__lblPath          = Label(self.__fmtSetting,
                                        text="path :",
                                        font=self.FONT_SMALL)
        self.__strPath          = StringVar()
        self.__entPath          = Entry(self.__fmtSetting,
                                        textvariable=self.__strPath,
                                        justify="left"
                                        )
        self.__strPath.trace_add("write", self.app.callbackPath)
        self.__btnRun           = Button(self.__fmtSetting,
                                        text="RUN",
    ...
    def windowStack(self) -> None:
        ...
        self.__lblStackMin      = Label(self.__fmtStackCtrl,
                                        text="min :",
                                        font=self.FONT_SMALL)
        self.__strStackMin      = StringVar(value=str(self.app.config.min))
        self.__entryStackMin    = Entry(self.__fmtStackCtrl,
                                        textvariable=self.__strStackMin,
                                        justify="center",
                                        width=self.ENT_CTRL_WIDTH,
                                        )
        self.__lblStackMax      = Label(self.__fmtStackCtrl,
                                        text="  max :",
                                        font=self.FONT_SMALL)
        self.__strStackMax      = StringVar(value=str(self.app.config.max))
        self.__entStackMax      = Entry(self.__fmtStackCtrl,
                                        textvariable=self.__strStackMax,
                                        justify="center",
                                        width=self.ENT_CTRL_WIDTH,
                                        )
        self.__lblStackSize     = Label(self.__fmtStackCtrl,
                                        text="  size :",
                                        font=self.FONT_SMALL)
        self.__strStackSize     = StringVar(value=str(self.app.config.size))
        self.__entStackSize     = Entry(self.__fmtStackCtrl,
                                        textvariable=self.__strStackSize,
                                        justify="center",
                                        width=self.ENT_CTRL_WIDTH,
                                        )
        self.__lblStackShuffle  = Label(self.__fmtStackCtrl,
                                        text="  shuffle :",
                                        font=self.FONT_SMALL)
        self.__intStackShuffle  = IntVar(value=True)
        self.__chkStackShuffle  = Checkbutton(self.__fmtStackCtrl,
                                        variable=self.__intStackShuffle,
                                        onvalue=1,
                                        offvalue=0
                                        )
        self.__btnStackGenerate = Button(self.__fmtStackCtrl,
                                        text="GENERATE",
        ...
        self.__entryStackMin.pack(side=LEFT, ipady=4)
        self.__lblStackMax.pack(side=LEFT)
        self.__entStackMax.pack(side=LEFT, ipady=4)
        self.__lblStackSize.pack(side=LEFT)
        self.__entStackSize.pack(side=LEFT, ipady=4)
        ...
        self.__entStackLst      = Entry(self.__fmtStackLst,
                                        textvariable=self.__strStackLst)
        self.__scrollStackLst   = Scrollbar(self.__fmtStackLst,
                                        orient="horizontal")
    ...
    @lstCmd.setter
    def lstCmd(self, l : List[str]) -> None:
